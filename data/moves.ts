...
export const Moves: {[moveid: string]: MoveData} = {
	...
	diffractingbeam: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, contact: 1, bypasssub: 1},
		secondary: null,
		target: "allAdjacent",
		type: "Electric",
		contestType: "Beautiful",
	...
	blindingshadow: {
		...
		pp: 10,
		flags: {contact: 1, protect: 1, mirror: 1, distance: 1, pulse: 1},
		onEffectiveness(typeMod, target, type, move) {
	...
	ashestoashes: {
		...
		onHit(target, source) {
			const item = target.takeItem(source);
			if (item) {
				this.add('-enditem', target, item.name, '[from] move: Ashes To Ashes', '[of] ' + source);
			} else {
	...
	psylash: {
		...
		onModifyMove(move) {
			if (this.field.isTerrain('psychicterrain') && source.isGrounded()) {
				this.debug('psychic terrain boost');
				return this.boost({def: -1}, source, target, null, true);
			}
		},
		secondary: null,
		target: "normal",
