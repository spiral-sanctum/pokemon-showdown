...
export const Moves: {[moveid: string]: MoveData} = {
	...
	asteroidbelt: {
		...
		condition: {
			onStart(pokemon) {
				this.add('-start', pokemon, 'Asteroid Belt');
			},
			onDamagingHitOrder: 6,
			onDamagingHit(damage, target, source, move) {
...
pressurize: {
	...
	bullethell: {
		...
		pp: 5,
		priority: 0,
		flags: {protect: 1, mirror: 1, neutral: 1},
		onModifyMove(move, pokemon) {
			move.allies = pokemon.side.pokemon.filter(ally => ally === pokemon || !ally.fainted && !ally.status);
			move.multihit = move.allies.length;
		},
	...
	coldconversion: {
		...
		self: {
			onHit(pokemon) {
				...
				this.add('-start', pokemon, 'typechange', pokemon.types.join('/'), '[from] move: Cold Conversion');
			},
		},
		secondary: null,
		target: "normal",
		...
		secondary: {
			...
			onHit(target, source) {
				...
				if (result === 0) {
					...
					target.tryAddVolatile('confusion', source);
				}
			},
		},
		target: "normal",
		...
		secondary: {
			...
			onHit(target, source) {
				...
				if (result === 0) {
					...
					target.tryAddVolatile('flinch', source);
				}
			},
		},
		target: "normal",
	...
	breach: {
		...
		onTry(source) {
			if (source.activeMoveActions > 1) {
				this.hint("Breach only works on your first turn out.");
				return false;
			}
		},
		secondary: null,
...
starfreeze: {
	...
	desertdance: {
		...
		flags: {snatch: 1},
		onModifyMove(move, pokemon) {
			if (['sandstorm'].includes(pokemon.effectiveWeather())) move.boosts = {spa: 2, spd: 2};
		},
		boosts: {
			spa: 1,
			spd: 1,
	...
	leafshield: {
		...
		flags: {snatch: 1},
		onModifyMove(move, pokemon) {
			if (['sunnyday', 'desolateland'].includes(pokemon.effectiveWeather())) move.boosts = {def: 2, spd: 2};
		},
		boosts: {
			atk: 1,
			spa: 1,
	...
	hiss: {
		...
		pp: 30,
		priority: 0,
		flags: {protect: 1, reflectable: 1, mirror: 1, sound: 1, authentic: 1, neutral: 1},
		boosts: {
			spd: -2,
		},
		secondary: null,
	...
	frostslash: {
		...
		pp: 20,
		priority: 0,
		flags: {protect: 1, mirror: 1, slash: 1, contact: 1},
		critRatio: 2,
		secondary: {
			chance: 10,
			status: 'frz',
	...
	dragonify: {
		...
		pp: 20,
		priority: 0,
		flags: {protect: 1, reflectable: 1, neutral: 1},
		onHit(target) {
	...
	lunarcannon: {
		...
		onBasePower(basePower, pokemon, target) {
			if (['raindance', 'sunnyday', 'desolateland', 'sandstorm', 'hail', 'wind', 'acidrain', 'sporestorm'].includes(pokemon.effectiveWeather())) {
				this.debug('weakened by weather');
				return this.chainModify(0.5);
			}
		},
	...
	powerscreen: {
		...
		condition: {
			...
			onStart(side) {
				this.add('-sidestart', side, 'Power Screen');
			},
			onResidualOrder: 21,
			onEnd(side) {
				this.add('-sideend', side, 'Power Screen');
			},
		},
		secondary: null,
		target: "allySide",
	...
	refract: {
		...
		condition: {
			...
			onStart(side) {
				this.add('-sidestart', side, 'Refract');
			},
			onResidualOrder: 21,
			onEnd(side) {
				this.add('-sideend', side, 'Refract');
			},
		},
		secondary: null,
		target: "allySide",
	...
	rocktumble: {
		...
		basePowerCallback(pokemon, target, move) {
			let bp = move.basePower;
			if (pokemon.volatiles['rocktumble'] && pokemon.volatiles['rocktumble'].hitCount) {
				bp *= Math.pow(2, pokemon.volatiles['rocktumble'].hitCount);
			}
			if (pokemon.status !== 'slp') pokemon.addVolatile('rocktumble');
			if (pokemon.volatiles['defensecurl']) {
				bp *= 2;
			}
			this.debug("Rock Tumble bp: " + bp);
			return bp;
		},
		category: "Physical",
		name: "Rock Tumble",
		...
		condition: {
			...
			onResidual(target) {
				if (target.lastMove && target.lastMove.id === 'struggle') {
					// don't lock
					delete target.volatiles['rocktumble'];
				}
			},
		},
		secondary: null,
	...
	perplex: {
		...
		basePower: 65,
		basePowerCallback(pokemon, target, move) {
			if (target.volatileStatus === 'confusion') return move.basePower * 2;
			return move.basePower;
		},
		category: "Special",
		name: "Perplex",
	...
	bluenote: {
		...
		pp: 10,
		priority: 0,
		flags: {sound: 1, protect: 1, reflectable: 1, mirror: 1},
		status: 'slp',
		secondary: null,
		target: "normal",
		type: "Sound",
	...
	corrupt: {
		...
		onAfterMove(pokemon, target, move) {
			if (move.mindBlownRecoil && !move.multihit) {
				this.damage(Math.round(pokemon.maxhp / 2), pokemon, pokemon, this.dex.getEffect('Corrupt'), true);
			}
		},
		secondary: null,
		target: "normal",
	...
	liquefy: {
		...
		onAfterMove(pokemon, target, move) {
			if (move.mindBlownRecoil && !move.multihit) {
				this.damage(Math.round(pokemon.maxhp / 2), pokemon, pokemon, this.dex.getEffect('Liquefy'), true);
			}
		},
		boosts: {
			spe: 2,
	...
	scorchedashes: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				...
				this.boost({atk: -1, spa: -1}, pokemon, this.effectState.source, this.dex.getActiveMove('scorchedashes'));
			},
		},
		secondary: null,
		pressureTarget: "self",
	...
	cursedwrath: {
		...
		basePower: 200,
		basePowerCallback(pokemon, target, move) {
			return move.basePower * pokemon.maxhp / pokemon.hp;
		},
		category: "Special",
		name: "Cursed Wrath",
		pp: 15,
	...
	puzzleroom: {
		...
		condition: {
			...
			durationCallback(source, effect) {
				...
				if (source?.hasAbility('trickster')) {
					this.add('-activate', source, 'ability: Trickster', effect);
					return 7;
				}
				return 5;
			},
	...
	nullroom: {
		...
		condition: {
			...
			onFieldStart(target, source) {
				this.add('-fieldstart', 'move: Null Room', '[of] ' + source);
			},
			onFieldRestart(target, source) {
				this.field.removePseudoWeather('nullroom');
			},
			// Item suppression implemented in Pokemon.ignoringItem() within sim/pokemon.js
			onFieldResidualOrder: 27,
			onFieldResidualSubOrder: 6,
			onFieldEnd() {
				this.add('-fieldend', 'move: Null Room', '[of] ' + this.effectState.source);
			},
		},
		secondary: null,
		target: "all",
	...
	singularity: {
		...
		condition: {
			...
			onFieldStart(target, source) {
				this.add('-fieldstart', 'move: Singularity', '[of] ' + source);
			},
			onEffectiveness(typeMod, target, type, move) {
				if (move.type == 'fire') return 0;
				if (move.type == 'water') return 0;
				if (move.type == 'ice') return 0;
				if (move.type == 'electric') return 0;
				if (move.type == 'grass') return 0;
				if (move.type == 'normal') return 0;
				if (move.type == 'psychic') return 0;
				if (move.type == 'dragon') return 0;
				if (move.type == 'fairy') return 0;
				if (move.type == 'steel') return 0;
				if (move.type == 'dark') return 0;
				if (move.type == 'ghost') return 0;
				if (move.type == 'ground') return 0;
				if (move.type == 'rock') return 0;
				if (move.type == 'flying') return 0;
				if (move.type == 'fighting') return 0;
				if (move.type == 'bug') return 0;
				if (move.type == 'sound') return 0;
				if (move.type == 'cosmic') return 0;
				if (move.type == 'poison') return 0;
			},
			onFieldRestart(target, source) {
				this.field.removePseudoWeather('singularity');
			},
			// Item suppression implemented in Pokemon.ignoringItem() within sim/pokemon.js
			onFieldResidualOrder: 27,
			onFieldResidualSubOrder: 6,
			onFieldEnd() {
				this.add('-fieldend', 'move: Singularity', '[of] ' + this.effectState.source);
			},
		},
		secondary: null,
		target: "all",
	...
	fiesta: {
		...
		condition: {
			// this is a side condition
			onSideStart(side) {
				this.add('-sidestart', side, 'Treats');
				this.effectState.layers = 1;
			},
			onSideRestart(side) {
				if (this.effectState.layers >= 2) return false;
			...
			onSwitchIn(pokemon) {
				if (!pokemon.isGrounded()) return;
				this.add('-sideend', pokemon.side, 'move: Treats', '[of] ' + pokemon);
	...
	amitywind: {
		...
		condition: {
			...
			onStart(target) {
				...
				this.add('-start', target, 'move: Amity Wind');
			},
			onResidualOrder: 15,
			onEnd(target) {
				this.add('-end', target, 'move: Amity Wind');
			},
			onDisableMove(pokemon) {
				for (const moveSlot of pokemon.moveSlots) {
					const move = this.dex.moves.get(moveSlot.id);
			...
			onBeforeMove(attacker, defender, move) {
				if (move.category !== 'Status') {
					this.add('cant', attacker, 'move: Amity Wind', move);
					return false;
				}
			},
		},
	...
	midnightterrain: {
		...
		condition: {
			...
			onModifyAccuracy(accuracy) {
				...
				if (pokemon.hasType('Dark') || pokemon.hasType('Ghost')) {
			 	...
			 	if (this.field.isTerrain('midnightterrain')) {
					this.debug('Midnight Terrain - decreasing accuracy');
					return accuracy * 0.9;
			 		}
				}
			},
			onBasePowerPriority: 6,
			onBasePower(basePower, attacker, defender, move) {
				if (move.type === 'Dark' || move.type === 'Ghost' && attacker.isGrounded() && !attacker.isSemiInvulnerable()) {
	...
	steamburst: {
		...
		onBasePower(basePower, pokemon, target) {
			if (['raindance'].includes(pokemon.effectiveWeather())) {
				this.debug('strenghtened by weather');
				return this.chainModify(2);
			}
		},
		secondary: null,
	...
	zealousdance: {
		...
		priority: 0,
		flags: {protect: 1, mirror: 1, dance: 1},
		volatileStatus: 'zealousdance',
		onHit(pokemon) {
			this.add('-activate', pokemon, 'move: Zealous Dance');
		},
		condition: {
			duration: 2,
			onRestart(pokemon) {
	...
	knightsvow: {
		...
		flags: {snatch: 1},
		onHit(target, source) {
			this.add('-activate', source, 'move: Knights Vow');
			let success = false;
			const allies = [...target.side.pokemon, ...target.side.allySide?.pokemon || []];
			for (const ally of allies) {
				if (ally.cureStatus()) success = true;
	...
	geosplinter: {
		...
		priority: 3,
		flags: {snatch: 1},
		volatileStatus: 'geosplinter',
		condition: {
			onStart(pokemon) {
				this.add('-start', pokemon, 'Geosplinter');
				const newatk = pokemon.storedStats.def;
				const newdef = pokemon.storedStats.atk;
				pokemon.storedStats.atk = newatk;
				pokemon.storedStats.def = newdef;
			...
			onEnd(pokemon) {
				this.add('-end', pokemon, 'Geosplinter');
				const newatk = pokemon.storedStats.def;
				const newdef = pokemon.storedStats.atk;
				pokemon.storedStats.atk = newatk;
				pokemon.storedStats.def = newdef;
			...
			onRestart(pokemon) {
				pokemon.removeVolatile('Geosplinter');
			},
		},
		secondary: null,
		target: "self",
